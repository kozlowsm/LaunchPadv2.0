...
for(let i = 0; i < launchDropDowns.length; i++) {
  launchDropDowns[i].addEventListener("click", event => {
    ...
    downArrowSymbols[i].setAttribute("fill", (downArrowSymbols[i].getAttribute("fill") == "#b0b1b3") ? "url(#red)" : "#b0b1b3");
    downArrowSymbols[i].setAttribute("filter", (downArrowSymbols[i].getAttribute("filter") == "none") ? "url(#dropshadow)" : "none");
  })
}
